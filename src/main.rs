    ...
    match args[1].as_str() {
        ...
        "add" => {
            ...
            if scan_db(package) {
                ...
                println!("Package '{}' is not in nixpkgs!", package);
            }
...
fn scan_db (package: &str) -> bool {
    let file = File::open("packages.db").expect("Couldn't open packages.db!");
    let reader = BufReader::new(file);

    for line in reader.lines() {
        if let Ok(line) = line {
            let line = line.trim_start_matches("* ");
            if line.starts_with("legacyPackages.") {
                if let Some(name) = line.strip_prefix("legacyPackages.x86_64-linux.") {
                    let final_name = name.split(' ').next().unwrap();
